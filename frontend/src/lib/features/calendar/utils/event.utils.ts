...
import { calendarStore } from '../stores/calendar.store.svelte';
...
export function ensureCalendarEvents(force: boolean = false, startDate?: Date, endDate?: Date): void {
  ...
  if (!force && !isDataStale && calendarStore.events && calendarStore.events.length > 0) {
    return;
  }
  
  if (calendarStore.calendars && calendarStore.calendars.length > 0) {
...
export function ensureCalendarData(force: boolean = false): void {
    ...
    calendarStore.fetchCalendars()
      .then(calendars => {
        ...
        if (calendars && calendars.length > 0 && 
            (force || isEventsStale || !calendarStore.events || calendarStore.events.length === 0)) {
  ...
  } else if (force || isEventsStale || !calendarStore.events || calendarStore.events.length === 0) {
    calendarStore.fetchEvents()
      .then(() => {
        lastEventsFetch = Date.now();
      });
