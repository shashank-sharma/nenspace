<script lang="ts">
    ...
    import { X, CheckCircle2, AlertCircle, Download, Loader2, Minus, Maximize2 } from 'lucide-svelte';
    import { formatFileSize } from '$lib/utils/file-validation.util';
    import { slide } from 'svelte/transition';

    const tasks = $derived(downloadProgressStore.tasks);
    ...
    function clearCompleted() {
        downloadProgressStore.clearCompleted();
    }

    function removeAll() {
        tasks.forEach(t => downloadProgressStore.removeTask(t.id));
    }
</script>
    ...
    <Card class="shadow-xl border-2 overflow-hidden flex flex-col pointer-events-auto bg-card/95 backdrop-blur-sm">
        <CardHeader class="p-3 border-b flex flex-row items-center justify-between space-y-0">
            <CardTitle class="text-sm font-bold flex items-center gap-2">
                ...
                Downloads ({completedCount}/{totalCount})
            </CardTitle>
                        ...
                        <div class="p-3 flex items-start gap-3 hover:bg-accent/50 transition-colors group">
                            <div class="mt-1 shrink-0">
                                ...
                                {:else if task.status === 'downloading'}
                                    <Download class="h-3.5 w-3.5 animate-bounce text-primary" />
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    <Button
                                        ...
                                        onclick={() => downloadProgressStore.removeTask(task.id)}
                                    >
                                        <X class="h-3 w-3" />
                                    </Button>
                                </div>
                                ...
                                <div class="mt-1 space-y-1">
                                    {#if task.status === 'downloading' || task.status === 'preparing'}
                                        ...
                                        <div class="flex justify-between text-[9px] text-muted-foreground">
                                            <span>{formatFileSize(task.bytesDownloaded)} / {task.totalBytes ? formatFileSize(task.totalBytes) : '?'}</span>
                                            <span>{task.progress}%</span>
                                        </div>
                                    {:else if task.status === 'completed'}
                                        <p class="text-[9px] text-green-600 font-medium">Completed</p>
                                    {:else if task.status === 'error'}
