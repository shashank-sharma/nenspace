...
import { 
    getNotificationBgTailwind, 
    getNotificationTextTailwind, 
    getNotificationIconTailwind,
    type NotificationVariant as NotificationVariantType
} from '../utils/notification-colors.util';
...
class IslandNotificationServiceImpl {
    ...
    #display(notification: IslandNotification): void {
        ...
        if (import.meta.env.DEV) {
            console.log(`[IslandNotificationService] Displaying notification: "${notification.message}"`);
        }
        ...
        if (browser) {
            emit('island-notification', {
                ...
                if (import.meta.env.DEV) {
                    console.log('[IslandNotificationService] Emitted to FloatingWidget');
                }
            }).catch(() => {
     ...
     * Uses centralized color utility for consistency
     */
    getVariantColors(variant: NotificationVariant): { bg: string; text: string; iconColor: string } {
        // Map 'custom' to 'default' for centralized utility
        const mappedVariant = variant === 'custom' ? 'default' : variant;
        
        return {
            bg: getNotificationBgTailwind(mappedVariant as NotificationVariantType),
            text: getNotificationTextTailwind(mappedVariant as NotificationVariantType),
            iconColor: getNotificationIconTailwind(mappedVariant as NotificationVariantType),
        };
    }
}
