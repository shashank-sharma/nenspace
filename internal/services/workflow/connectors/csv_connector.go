...
func (c *CSVConnector) readCSV(filePath string, config map[string]interface{}) (map[string]interface{}, error) {
	...
	schema.SourceNodes = []string{nodeID}
...
func (c *CSVConnector) writeCSV(filePath string, config map[string]interface{}, input map[string]interface{}) (map[string]interface{}, error) {
	...
	// Debug logging
	logger.Info.Printf("[CSV Destination] Input keys: %v", getKeys(input))
	logger.Info.Printf("[CSV Destination] Envelope data length: %d", len(envelope.Data))
	if dataRaw, ok := input["data"]; ok {
		logger.Info.Printf("[CSV Destination] Raw data type: %T, value: %v", dataRaw, dataRaw)
	}
	...
	if includeHeader && (!appendMode || fileInfo.Size() == 0) {
	...
	// Preserve input schema in output envelope to allow schema propagation
	outputSchema := envelope.Metadata.Schema
	if len(outputSchema.Fields) == 0 && len(headers) > 0 {
		// If no input schema but we have headers, create schema from headers
		outputSchema.Fields = make([]types.FieldDefinition, 0, len(headers))
		for _, header := range headers {
	...
	outputEnvelope := &types.DataEnvelope{
		...
		Metadata: types.Metadata{
			...
			ExecutionTimeMs: 0,
			Schema:          outputSchema, // Preserve schema for propagation
...
// GetOutputSchema returns the schema for CSV connector
// For CSV source, tries to infer schema from file headers
// For CSV destination, preserves input schema
func (c *CSVConnector) GetOutputSchema(inputSchema *types.DataSchema) (*types.DataSchema, error) {
	if c.Type() == types.SourceConnector {
		...
		// Try to read file and infer schema from headers
		filePath, ok := c.Config["file_path"].(string)
		if !ok || filePath == "" {
			// Can't infer schema without file path
		...
		// Try to read first few lines to infer schema
		schema, err := c.inferSchemaFromFile(filePath)
		if err != nil {
			// If we can't read the file, return empty schema
			// It will be populated during Execute()
			return &types.DataSchema{
				Fields:      make([]types.FieldDefinition, 0),
				SourceNodes: make([]string, 0),
			}, nil
		...
		// For destination, preserve input schema if available
		// This allows schema to propagate through CSV nodes
		if inputSchema != nil {
			return inputSchema, nil
		}
		// If no input schema, return empty (destinations don't output data)
		return &types.DataSchema{
			Fields:      make([]types.FieldDefinition, 0),
			SourceNodes: make([]string, 0),
		}, nil
...
// inferSchemaFromFile attempts to read the CSV file and infer schema from headers
// This is used by GetOutputSchema to provide schema information at design time
func (c *CSVConnector) inferSchemaFromFile(filePath string) (*types.DataSchema, error) {
	resolvedPath := c.resolveFilePath(filePath)
	...
	hasHeader, _ := c.Config["has_header"].(bool)
	nodeID := c.ID()
